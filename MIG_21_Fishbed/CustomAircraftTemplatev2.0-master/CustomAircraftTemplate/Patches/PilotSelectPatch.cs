...
*/
        public static IEnumerator BetterSetupCampaignScreenRoutine(CampaignSelectorUI instance)
        {
            ...
            //Debug.Log("F4PII BSCSCR Patch 1.2");
            ...
            //Debug.Log("F4PII BSCSCR Patch 1.3");
            if (_campaignsParent)
            {
                //Debug.Log("F4PII BSCSCR Patch 1.4");
            ...
            //Debug.Log("F4PII BSCSCR Patch 1.5");
            ...
            //Debug.Log("F4PII BSCSCR Patch 1.6" + PilotSaveManager.currentVehicle.vehicleName);
            ...
            //Debug.Log("F4PII BSCSCR Patch 1.6.0.1" + _aircraft[0]);

            {
                //Debug.Log("F4PII BSCSCR Patch 1.6.1");
                _aircraft.Add("F/A-26B");


            }
            //Debug.Log("F4PII BSCSCR Patch 1.7");
            foreach (var vName in _aircraft)
            {
                //Debug.Log("F4PII BSCSCR Patch 1.8: " + vName);
            ...
            /* ---- END EDIT ---- */
            sw.Stop();
            //Debug.Log("F4PII [CSIPatch] Time loading BuiltInCampaigns: " + sw.ElapsedMilliseconds);
            sw.Reset();
            sw.Start();
            //Debug.Log("F4PII BSCSCR Patch 1.11");

            foreach (var vName in _aircraft)
            {
                //Debug.Log("F4PII BSCSCR Patch 1.11.1: " + vName);
                if (PilotSaveManager.GetVehicle(vName).campaigns != null)
                {
                    foreach (var campaign in PilotSaveManager.GetVehicle(vName).campaigns)
                    {
                        //Debug.Log("F4PII BSCSCR Patch 1.12: " + campaign + ", " + vName);

                        if (campaign.isSteamworksStandalone)
                        {
                            //Debug.Log("F4PII BSCSCR Patch 1.13");
                            _campaigns.Add(campaign);
                            if (!SteamClient.IsValid) continue;
                            pubCampaign = campaign;
                            traverse.Field("swStandaloneCampaign").SetValue(campaign);
                            //Debug.Log("F4PII BSCSCR Patch 1.14");
                        ...
                        else if (campaign.readyToPlay)
                        {
                            ...
                            //Debug.Log("F4PII BSCSCR Patch 1.18");
            ...
            sw.Stop();
            //Debug.Log("F4PII [CSIPatch] Time loading vehicle campaigns: " + sw.ElapsedMilliseconds.ToString());
            sw.Reset();
            sw.Start();
            VTResources.GetCustomCampaigns();
            sw.Stop();
            //Debug.Log("F4PII [CSIPatch] Time loading custom campaigns list: " + sw.ElapsedMilliseconds.ToString());
            sw.Reset();
            sw.Start();
            ...
            foreach (var vName in _aircraft)
            {
                //Debug.Log("F4PII BSCSCR Patch 1.19: " + vName);
            ...
            sw.Stop();
            //Debug.Log("F4PII [CSIPatch] Time converting custom campaigns ToIngameCampaigns: " + sw.ElapsedMilliseconds);
            sw.Reset();
            int num;
            //Debug.Log("F4PII BSCSCR Patch 1.22");
            for (var i = 0; i < _campaigns.Count; i = num + 1)
            {
                //Debug.Log("F4PII BSCSCR Patch 1.23");
            ...
            //Debug.Log("F4PII BSCSCR Patch 1.28");
            ...
            if (setupList == null)
            {
                //Debug.Log("F4PII BSCSCR Patch 1.28.1");
            }
    ...
    public static class F4PII_CSIPatch_FinallyOpenCampaignSelector
    {
        ...
        {
            //Debug.unityLogger.logEnabled = Main.logging;
            //Debug.Log("F4PII CSI Patch 1.1");
            var traverse = Traverse.Create(instance);
            instance.campaignDisplayObject.SetActive(true);
            instance.missionBriefingDisplayObject.SetActive(false);
            //Debug.Log("F4PII CSI Patch 1.2");
            yield return instance.StartCoroutine(CSIPatchUtils.BetterSetupCampaignScreenRoutine(instance));
            //Debug.Log("F4PII CSI Patch 1.2.1.1");
            if (onOpenedSelector != null)
            {
                //Debug.Log("F4PII CSI Patch 1.3");
                onOpenedSelector();
                yield break;
            }
            //Debug.Log("F4PII CSI Patch 1.2.1.2");
            try
            {
                //Debug.Log("F4PII CSI Patch 1.2.1.2.1");
                String1 = PilotSaveManager.currentCampaign.campaignName;
                //Debug.Log("F4PII CSI Patch 1.2.1.2.2");
            }
            
            catch (NullReferenceException)
            {
                //Debug.Log("F4PII CSI Patch 1.2.1.2.4");
            ...
            //Debug.Log("F4PII CSI Patch 1.2.1.3");
            if (String1 == null) { yield break; }
            //Debug.Log("F4PII CSI Patch 1.2.1" + PilotSaveManager.currentCampaign.campaignName);
            if (!PilotSaveManager.currentCampaign) yield break;
            //Debug.Log("F4PII CSI Patch 1.4");
            ...
            //Debug.Log("F4PII CSI Patch 1.5");
            if (num >= 0)
            {
                //Debug.Log("F4PII CSI Patch 1.6");
                ...
                if (num2 >= 0)
                {
                    //Debug.Log("F4PII CSI Patch 1.10" + num2);
                    //this.missionIdx = num2;
                    traverse.Field("missionIdx").SetValue(num2);
                    //Debug.Log("F4PII CSI Patch 1.10.1");
                    instance.MissionsButton();
                    //Debug.Log("F4PII CSI Patch 1.10.2");
                }
                else
                {
                    //Debug.Log("F4PII CSI Patch 1.11");
                    var numa = _campaigns[(int)_campaignIdx.GetValue()];
                    //Debug.Log("F4PII CSI Patch 1.11.0");
                    num2 = numa.trainingMissions
                        .FindIndex(x => PilotSaveManager.currentScenario.scenarioID == x.scenarioID);
                    //Debug.Log("F4PII CSI Patch 1.11.1" + num2) ;
                    if (num2 < 0) yield break;
                    //this.trainingIdx = num2;
                    //Debug.Log("F4PII CSI Patch 1.12");
            ...
            {
                //Debug.Log("F4PII CSI Patch 1.13");
                ...
                {
                    //Debug.Log("F4PII CSI Patch 1.14");
                    while (enumerator.MoveNext())
                    {
                        //Debug.Log("F4PII CSI Patch 1.15");
                        ...
                        //Debug.Log("F4PII CSI Patch 1.16");
                        var _viewingCampaign = (Campaign)traverse.Field("viewingCampaign").GetValue();
                        while (_viewingCampaign == null)
                        {
                            //Debug.Log("F4PII CSI Patch 1.17");
                            _viewingCampaign = (Campaign)traverse.Field("viewingCampaign").GetValue();
                            yield return null;
                        }
                        if (lastCSave == null)
        ...
        {
            //Debug.unityLogger.logEnabled = Main.logging;
            //Debug.Log("F4PII CSI Patch 1.20: cv = " + PilotSaveManager.currentVehicle.vehicleName + ", ModVN = " + AircraftInfo.vehicleName);

            if (PilotSaveManager.currentVehicle.vehicleName != AircraftInfo.vehicleName) { return; }
            //Debug.Log("F4PII CSI Patch 1.20");
            instance = __instance;
            //Debug.Log("F4PII CSI Patch 1.21");
            instance.StartCoroutine(CSI_FinallyOpenCampaignSelectorRoutine(onOpenedSelector));
            //Debug.Log("F4PII CSI Patch 1.22");
        }
    }

    [HarmonyPatch(typeof(CampaignSelectorUI), "StartWorkshopCampaign")]
    class F4PII_CSIPatch_StartWorkshopCampaign
    {
        static CampaignSelectorUI instance;
        ...
        static IEnumerator CSI_StartWorkshopCampaignRoutine(string campaignID)
        {
            ...
            //Debug.Log("F4PII CSI Patch 2.2");
            var campaign = VTResources.GetSteamWorkshopCampaign(campaignID);
            if (campaign == null)
            {
                //Debug.Log("F4PII CSI Patch 2.3");
                //Debug.Log("F4PII Missing campaign in CSI_StartWorkshopCampaignRoutine");
            }
            ...
            //Debug.Log("F4PII CSI Patch 2.8");
            instance.campaignDisplayObject.SetActive(true);
            PilotSaveManager.currentCampaign = campaign2;
            //Debug.Log("F4PII CSI Patch 2.9");
        ...
        {
            if (PilotSaveManager.currentVehicle.vehicleName == AircraftInfo.vehicleName) { return true; }
            //Debug.Log("F4PII CSI Patch 2.0.1");
            {
                //Debug.Log("F4PII CSI Patch 2.0.2");
                instance = __instance;
    ...
    class F4PII_CSIPatch_StartWorkshopMission
    {
        static CampaignSelectorUI instance;
        ...
        {
            //Debug.unityLogger.logEnabled = Main.logging;
            //Debug.Log("F4PII CSI Patch 3.0.1");
            var traverse = Traverse.Create(instance);
            ...
            //Debug.Log("F4PII CSI Patch 3.6");
            foreach (var mission in campaign.missions)
            {
                //Debug.Log("F4PII CSI Patch 3.6.1");
                ...
                //Debug.Log("F4PII CSI Patch 3.6.2");
